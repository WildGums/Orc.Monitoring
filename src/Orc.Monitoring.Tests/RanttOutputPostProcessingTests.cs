...
namespace Orc.Monitoring.Tests
{
    ...
    public class RanttOutputPostProcessingTests
    {
        ...
        public void Setup()
        {
            ...
            _autoMocker.Use(_postProcessorMock.Object);

            // Mock MonitoringController static methods
            _autoMocker.Setup<MonitoringController, EnhancedDataPostProcessor>(x => MonitoringController.GetEnhancedDataPostProcessor())
                .Returns(_postProcessorMock.Object);
            _autoMocker.Setup<MonitoringController, EnhancedDataPostProcessor.OrphanedNodeStrategy>(x => MonitoringController.GetOrphanedNodeStrategy())
                .Returns(EnhancedDataPostProcessor.OrphanedNodeStrategy.AttachToNearestAncestor);

            _ranttOutput = new RanttOutput(_loggerMock.Object);
        ...
        public async Task ExportToCsv_WithOrphanedNodes_AppliesPostProcessing()
        {
            ...
            StringAssert.Contains("OrphanedMethod", csvContent);
            StringAssert.Contains("1,3,", csvContent); // Check if orphaned node is attached to "1"
        ...
        public async Task ExportRelationships_EnsuresRootNodeExists()
        {
            ...
            StringAssert.Contains("ROOT,1,", relationshipsContent);
            StringAssert.Contains("1,2,", relationshipsContent);
        }
        ...
        public async Task ExportToCsv_WithDifferentOrphanedNodeStrategies_AppliesCorrectStrategy()
        {
            ...
            _autoMocker.Setup<MonitoringController, EnhancedDataPostProcessor.OrphanedNodeStrategy>(x => MonitoringController.GetOrphanedNodeStrategy())
                .Returns(EnhancedDataPostProcessor.OrphanedNodeStrategy.RemoveOrphans);

            await InitializeAndExportData(reportItems);

            var csvFilePath = Path.Combine(_testOutputPath, "TestReporter", "TestReporter.csv");
            var csvContent = await File.ReadAllTextAsync(csvFilePath);
            StringAssert.Contains("Method1", csvContent);
            StringAssert.DoesNotContain("OrphanedMethod", csvContent);

            // Test AttachToRoot strategy
            _autoMocker.Setup<MonitoringController, EnhancedDataPostProcessor.OrphanedNodeStrategy>(x => MonitoringController.GetOrphanedNodeStrategy())
                .Returns(EnhancedDataPostProcessor.OrphanedNodeStrategy.AttachToRoot);
